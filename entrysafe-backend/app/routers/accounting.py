...
@api_router.post("/reject-transaction")
async def reject_transaction(
    request: dict,
    db=Depends(get_database)
):
    """
    Reject and delete a previewed transaction.
    """
    ...
    try:
        ...
        # Find and delete the preview
        result = await db.transaction_previews.delete_one({
            "id": preview_id
        })

        if result.deleted_count == 0:
            logger.warning(f"Preview {preview_id} not found or already deleted")
            # Return success anyway - idempotent operation

        return {
            "status": "rejected",
            "message": "Transaction preview rejected and deleted"
        }
